...
class PlazaLayoutTest {
    @Test fun `plaza header follows Figma geometry`() {
        ...
        assertEquals(context.dp(68),(header.title.layoutParams as android.widget.FrameLayout.LayoutParams).marginStart)
        assertEquals(context.dp(33),header.action.layoutParams.height)
    }
    @Test fun `recycling a comment as a post restores content alignment`() {
        val context=ContextThemeWrapper(RuntimeEnvironment.getApplication(),R.style.AppTheme)
        ...
        val body=(0 until view.childCount).map(view::getChildAt).filterIsInstance<TextView>().first {it.text=="Body"}
        assertEquals(context.dp(44),(body.layoutParams as android.widget.LinearLayout.LayoutParams).marginStart)
    ...
    @Test fun `detail counters update and footer stays within a narrow large font screen`() {
        val app=RuntimeEnvironment.getApplication()
        val config=android.content.res.Configuration(app.resources.configuration).apply {fontScale=2f}
        val context=ContextThemeWrapper(app.createConfigurationContext(config),R.style.AppTheme)
        val footer=PlazaReplyBar(context,{}, {},{})
        footer.bind(PlazaPost(1,42,"Author","Body",1,null,null,null,9999,8888,false,emptyList()))
        footer.measure(View.MeasureSpec.makeMeasureSpec(context.dp(320),View.MeasureSpec.EXACTLY),View.MeasureSpec.makeMeasureSpec(0,View.MeasureSpec.UNSPECIFIED))
    ...
    @Test fun `nine image grid measures in narrow wide dark and large font configurations`() {
        for (dark in listOf(false,true)) for (widthDp in listOf(320,720)) {
            ...
            val context = ContextThemeWrapper(app.createConfigurationContext(config), R.style.AppTheme)
            val view = PostView(context) { 42L }
            val post = PlazaPost(1,42,"Long author name 很长的用户名", "A long post 正文内容".repeat(30),1,
                123,"manga",null,8,9,false,(1..9).map {
                    PlazaImage("id-$it",960,1200,"image/jpeg","https://example.invalid/$it",999999) })
            view.bind(post,false,false,{}, {}, {_,_->})
            val width = context.dp(widthDp)
            fun layout() { view.measure(View.MeasureSpec.makeMeasureSpec(width,View.MeasureSpec.EXACTLY),
                View.MeasureSpec.makeMeasureSpec(0,View.MeasureSpec.UNSPECIFIED)); view.layout(0,0,width,view.measuredHeight) }
            layout()
            ...
            for (r in 0 until grid.childCount) {
                ...
                for (c in 0 until row.childCount) {
                    ...
                    assertTrue(tile.measuredWidth > 0); assertTrue(tile.measuredHeight > 0)
