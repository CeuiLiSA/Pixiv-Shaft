...
class MediaPerformanceTest {
    @Test fun `fast transfer emits initial and final progress instead of a callback per chunk`() {
    ...
    @Test fun `slow transfer remains responsive and always reports final bytes`() {
        val progress = MediaUploadProgress()
    ...
    @Test fun `Retrofit timing tag stays local and complete still returns preview in one response`() = runBlocking {
        MockWebServer().use { server ->
            ...
            val api = Retrofit.Builder().baseUrl(server.url("/"))
                .addConverterFactory(GsonConverterFactory.create()).client(client).build().create(MediaApi::class.java)
            server.enqueue(MockResponse().setHeader("Server-Timing", "sign;dur=0.20, total;dur=2.10")
                .setBody("""{"mediaId":"test","objectKey":"image.jpg","contentType":"image/jpeg","size":12,"width":4,"height":3,"createdAt":1,"url":"https://media.pixshaft.com/image.jpg?q-signature=test","expiresAt":1700000300000}"""))
            val response = api.completeUpload(MediaUploadCompleteRequest("test", "image.jpg", "image/jpeg", 12,
                width = 4, height = 3), trace)
            assertTrue(tagged)
    ...
    @Test fun `COS warmup reuses its connection without modifying the following signed PUT`() {
        MockWebServer().use { server ->
            ...
            client.newBuilder().build().newCall(Request.Builder().url(signed)
                .header("x-cos-forbid-overwrite", "true")
                .put("exact bytes".toRequestBody()).build()).execute().use { assertEquals(200, it.code) }
