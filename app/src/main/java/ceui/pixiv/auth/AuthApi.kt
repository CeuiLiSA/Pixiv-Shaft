...
internal interface AuthApi {
    ...
    @POST("v1/auth/logout") fun logout(@Body body: LogoutRequest): Call<Unit>
}
...
internal fun TokenResponse.toSession(
    ...
    if (accessExpiresAt <= 0L || refreshExpiresAt <= accessExpiresAt || generation <= 0L)
        return null
    return AuthSession(
        uid = uid,
        sessionId = sessionId,
...
/**
 * Naked auth client. It must never inherit the main client's authenticator: a 401 from the token
 * endpoint cannot be allowed to recursively refresh itself.
 */
internal object AuthNetwork {
    ...
    private fun createApi(baseUrl: String): AuthApi {
        val client =
            (if (baseUrl == ClientManager.MEDIA_API_HOST)
                    ceui.pixiv.shaftapi.MediaHttpTransport.apiClient.newBuilder()
                ...
                .addInterceptor { chain ->
                    ...
                    if (
                        !request.url.encodedPath.endsWith("/v1/auth/session") ||
                            body == null ||
                            !ShaftHmac.isConfigured
                    ) {
