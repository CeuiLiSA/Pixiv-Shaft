...
/**
 * First-party PixShaft session. This is deliberately separate from [SessionManager], which owns
 * Pixiv's OAuth tokens and its unusual HTTP-400 refresh contract.
 */
...
object MediaAuthSessionManager :
    BackendSessionManager(
        { AuthNetwork.mediaApi },
        { TokenStore("pixshaft-media-auth-v2") },
    )

open class BackendSessionManager
internal constructor(
    ...
    @Volatile private var current: AuthSession? = null

    @Volatile private var nextBootstrapAtElapsed: Long = 0L
    ...
    fun logoutCurrentSession() {
        val session =
            synchronized(loadLock) {
                ...
                runCatching { tokenStore.clear() }
                    .onFailure {
                        AuthLog.warning("failed to clear local auth session during logout", it)
                    }
                snapshot
            } ?: return
        ...
        logoutScope.launch {
            runCatching {
                ...
                if (response.isSuccessful) {
                    AuthLog.debug(
                        "server logout completed uid=${session.uid} status=${response.code()}"
                    )
                } else {
                    AuthLog.warning(
                        "server logout rejected uid=${session.uid} status=${response.code()}"
                    )
                }
            }
                .onFailure {
                    AuthLog.warning("server session logout failed uid=${session.uid}", it)
                }
        }
    }
    ...
    private fun performRefresh(): String? {
        val (session, expectedStateVersion) =
            synchronized(loadLock) {
        ...
        val attemptId =
            runCatching {
                ...
                .onFailure {
                    AuthLog.warning(
                        "refresh stopped before network; attempt was not durable generation=${session.generation}",
                        it,
                    )
                }
        ...
        return try {
            ...
            if (response.isSuccessful) {
                ...
                if (
                    fresh == null ||
                        ...
                        fresh.generation <= session.generation
                ) {
                    AuthLog.warning(
                ...
                persistAndPublish(fresh, expectedStateVersion)
                    ?.also {
                        AuthLog.debug(
                            "session refresh published uid=$uid generation=${it.generation}"
                        )
                    }
                    ?.accessToken
            } else {
            ...
            AuthLog.warning(
                "session refresh failed transiently uid=$uid generation=${session.generation}",
                t,
            )
            null
        }
