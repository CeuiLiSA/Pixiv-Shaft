...
internal class PlazaReplyBar(
    ...
    private val input =
        context.label(context.getString(R.string.plaza_comment_hint), 16f).apply {
            maxLines = 1
    ...
    init {
        ...
        fun counter(
            ...
            row.addView(
                ImageView(context).apply {
                    ...
                    imageTintList =
                        ColorStateList.valueOf(V3Palette.from(context).floatingPillContent)
                },
                LayoutParams(context.dp(24), context.dp(24)),
            )
        ...
        counter(
            ...
            reactionCount,
            context.getString(R.string.plaza_react_post),
            react,
        )
        counter(
            R.drawable.ic_plaza_figma_comment,
            replyCount,
            context.getString(R.string.plaza_view_comments),
            comments,
        )
    }
    ...
    fun bind(post: PlazaPost?, busy: Boolean = false) {
        ...
        reactionCount.text =
            compact((post?.likeCount ?: 0) + (post?.reactions?.sumOf { it.count } ?: 0))
        ...
        for (i in 0 until childCount) getChildAt(i).layoutParams =
            getChildAt(i).layoutParams.apply { this.height = height }
    }

    private fun compact(count: Int) =
