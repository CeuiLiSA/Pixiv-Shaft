...
class PlazaImageViewer : DialogFragment() {
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val close = ctx.action(ctx.getString(R.string.plaza_close))
        close.setTextColor(Color.WHITE)
        close.background = null
        val status =
            ctx.label(ctx.getString(R.string.plaza_loading)).apply {
        ...
        fun load(force: Boolean = false) {
            if (loading) return
            loading = true
            status.text = ctx.getString(R.string.plaza_loading)
                ...
                try {
                    val cached = runCatching {
                        com.google.gson
                            .Gson()
                            .fromJson(
                                requireArguments().getString("images"),
                                Array<PlazaImage>::class.java,
                            )
                            .toList()
                    }
                    ...
                    val images =
                        if (
                            !force &&
                                cached.isNotEmpty() &&
                                cached.all { it.expiresAt > System.currentTimeMillis() + 5000 }
                        )
                            cached
                        else PlazaRepository.api.post(requireArguments().getLong("post")).images
                    pager.adapter =
                        object : RecyclerView.Adapter<ImageHolder>() {
                            ...
                            override fun onBindViewHolder(holder: ImageHolder, position: Int) {
                                ...
                                holder.photo.contentDescription =
                                    ctx.getString(
                                        R.string.plaza_image_zoom_accessibility,
                                        position + 1,
                                    )
                                imageRequests
                                    .load(
                                        images[position]
                                            ...
                                            ?.let {
                                                PlazaMediaUrl(
                                                    images[position],
                                                    ceui.pixiv.session.SessionManager.loggedInUid,
                                                )
                                            }
                                    )
                                    .dontAnimate()
                            ...
                            override fun onViewRecycled(holder: ImageHolder) {
                                imageRequests.clear(holder.photo)
                            }
                        }
                    ...
                    pager.setCurrentItem(
                        (savedInstanceState?.getInt("index") ?: requireArguments().getInt("index"))
                            .coerceIn(0, (images.size - 1).coerceAtLeast(0)),
                        false,
                    )
                    status.text =
                        if (images.isEmpty()) ctx.getString(R.string.plaza_no_images)
                        else
                            ctx.getString(
                                R.string.plaza_image_position,
                                pager.currentItem + 1,
                    ...
                    ctx.showPlazaError(plazaError(e))
                    status.text =
                        ctx.getString(R.string.plaza_retry_message, plazaError(e).resolve(ctx))
                } finally {
                    loading = false
                }
            }
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putInt(
            "index",
            dialog?.findViewById<ViewPager2>(ceui.lisa.R.id.plaza_image_pager)?.currentItem ?: 0,
        )
    }
    ...
    companion object {
        fun newInstance(post: ceui.pixiv.plaza.PlazaPost, index: Int) =
            PlazaImageViewer().apply {
                arguments =
                    bundleOf(
                        "post" to post.id,
                        "index" to index,
                        "images" to com.google.gson.Gson().toJson(post.images),
                    )
            }
    }
}
...
/**
 * Fit-center decoding stays bounded by the screen; pinch zoom does not load an original-size
 * bitmap.
 */
private class ZoomImageView(context: android.content.Context) :
