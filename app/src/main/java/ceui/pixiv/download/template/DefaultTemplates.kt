...
object DefaultTemplates {
    ...
    // 合并下载的合集文件。文件名沿用旧版 IllustDownload.downloadNovel(NovelSeriesItem)
    // 写死的那套 `NovelSeries_<id>_Chapter_1~N_<系列名>.txt`——issue #964 的用户点名
    // 要这一版（比 V3 那套 `<系列名>_合集_ID<id>` 多一个章节数）。目录另起
    // `Shaft/Novels/`（不跟单篇小说的 legacy `ShaftNovels/` 混），也不再跟着小说
    // 模板渲染结果走，独立可改。
