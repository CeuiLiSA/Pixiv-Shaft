...
internal object MediaHttpTransport {
    val apiClient: OkHttpClient = OkHttpClient.Builder()
        .eventListenerFactory(MediaNetworkTiming.factory)
        .build()
    val storageClient: OkHttpClient = OkHttpClient.Builder()
        .eventListenerFactory(MediaNetworkTiming.factory)
        .build()
    ...
    internal fun warm(client: OkHttpClient, url: String, running: AtomicBoolean) {
        if (client.connectionPool.connectionCount() > 0 || !running.compareAndSet(false, true)) return
        val trace = MediaUploadTrace("prewarm")
        val request = Request.Builder().url(url).head().tag(MediaUploadTrace::class.java, trace).build()
        trace.stage("connection", "host=${request.url.host}")
        client.newBuilder().callTimeout(3, TimeUnit.SECONDS).build()
            .newCall(request).enqueue(object : Callback {
                ...
                override fun onResponse(call: Call, response: Response) {
                    // A private COS root may answer 403/404; its HTTPS connection is still reusable.
