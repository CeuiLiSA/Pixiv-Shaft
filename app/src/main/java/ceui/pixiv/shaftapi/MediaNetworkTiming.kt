...
internal class MediaNetworkTiming(private val trace: MediaUploadTrace) : EventListener() {
    ...
    override fun dnsStart(call: Call, domainName: String) {
        dnsAt = System.nanoTime()
    }

    override fun dnsEnd(call: Call, domainName: String, inetAddressList: List<InetAddress>) {
        trace.event(
            "dns",
            "host=$domainName dnsMs=${ms(dnsAt)} addressCount=${inetAddressList.size}",
        )
    }
    ...
    override fun connectionAcquired(call: Call, connection: Connection) {
        trace.event(
            "connection",
            "host=${call.request().url.host} reused=${!connected} protocol=${connection.protocol()}",
        )
    }

    override fun requestHeadersEnd(call: Call, request: okhttp3.Request) {
        headersAt = System.nanoTime()
    }

    override fun requestBodyStart(call: Call) {
        bodyAt = System.nanoTime()
    }
    ...
    override fun responseHeadersEnd(call: Call, response: Response) {
        trace.event(
            "response_headers",
            "http=${response.code} headersToResponseMs=${ms(headersAt)}",
        )
