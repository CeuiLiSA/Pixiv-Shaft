...
class IllustCardViewHolder(bd: CellIllustCardBinding) : ListItemViewHolder<CellIllustCardBinding, IllustCardHolder>(bd) {
    ...
    override fun onBindViewHolder(holder: IllustCardHolder, position: Int) {
        ...
        val itemHeight = (itemWidth * holder.illust.height / holder.illust.width.toFloat()).roundToInt()
        binding.image.updateLayoutParams {
            width = itemWidth
            height = itemHeight
        }

        Glide.with(binding.root.context).load(GlideUrlChild(holder.illust.image_urls?.large)).into(binding.image)
        binding.image.setOnClick {
            it.findActionReceiverOrNull<IllustCardActionReceiver>()?.onClickIllustCard(holder.illust)
        }
    }
}