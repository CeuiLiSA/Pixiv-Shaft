 ...
 *（clearNovelWatchLater 按 entityType 删，插画 tab 不受影响）。
 */
...
/**
 * 小说稍后再看的数据源：general_table(WATCH_LATER + NOVEL) 全量单页，没有翻页
    ...
    override suspend fun load(cursor: String?): FeedPage<String> {
        val items: List<FeedItem> = withContext(Dispatchers.IO) {
            AppDatabase.getAppDatabase(Shaft.getContext()).generalDao()
                .getByRecordTypeAndEntityType(RecordType.WATCH_LATER, EntityType.NOVEL, 0, Int.MAX_VALUE)
                .mapNotNull { entity ->
