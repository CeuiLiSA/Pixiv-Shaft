 ...
 *
 * 两者不是替代关系，而是分工：本页是**入口**（本地库一注册就能进，第一页在网络 RTT 内落库，
 * 之后每 5 秒一页；补齐期间只允许默认排序浏览，筛选/排序/搜索会被收走 —— 见 [BookmarkLibraryUi]），
 * 原列表退居**兜底**：没开镜像、离线且从没镜像过、以及右上角「原始收藏列表」那个出口
 * （要看服务端原序、或者就要立刻翻到底）。所以本页在补齐之前会挂一条进度条老实说「还在补」，
 * 补齐之后那条就永远消失。
 *
 * ## 本类只剩下「我是插画版」这一件事
 *
