...
object SessionManager {
    ...
    /**
     * 网页版会话是否已同步。OAuth 登录只拿到 app-api 的 token，网页 cookie 得用户在 「Web 首页」里登录一次才写进来（见
     * [ceui.pixiv.ui.web.WebFragment]）。 匿名身份下 www.pixiv.net 的 ajax 只返回全年龄作品，所以依赖网页接口的功能 （黑名单、按 tag
     * 筛画师作品…）要先判这一条。
     */
     ...
     *
     * 判据不能是「含 PHPSESSID」：pixiv 在登录页刚打开、用户还没输账号密码时就会先发一个 **匿名** PHPSESSID（纯 32 位 hex）。按含即算数的话，登录
     * WebView 会在 onPageFinished 的第一帧就判成功并被拆掉，用户根本来不及登录；而存下来的匿名 cookie 又让 [hasWebCookie]
     * 恒真，反过来把「去网页登录一次」的引导也一并关掉。
     *
     ...
     *
     * 它会把 `.pixiv.net` 和 `www.pixiv.net` 两个域下的同名 cookie 一并吐出来，登录前后 各写过一次 PHPSESSID 时就并排出现两条： `…;
     * PHPSESSID=<匿名hash>; …; PHPSESSID=<uid>_<hash>; …`。 服务端只认先出现的那条，于是「明明带着登录 cookie，却仍被当匿名」——画师按
     * tag 筛选 恒 0 件就是这么来的。
     *
     * 同名取最后一条（新值覆盖旧值）；PHPSESSID 额外保护：已经收下登录态的那条后， 不再被后面的匿名值盖回去。
     */
    ...
    /**
     * Migrate user data from old SharedPreferences (Local.getUser()) to MMKV. Only migrates when
     * SessionManager has no data AND SharedPreferences has data.
     */
     ...
     *
     * uid 和时刻合在**一个**引用里，而不是两个 @Volatile 字段：读方在 IO 线程 （[premiumAgeMs] 由上报队列在发送那一刻调），写方在主线程，分开存的话 读完
     * uid 到读时刻之间插进一次**切账号后的观测**，就会把旧号的 uid 配上 新号的时刻，报出一个比任何真实观测都新的年龄 —— 而服务端正是拿「比那次
     * 拒绝更新」当证据把号放回池子的。一起写、一起读，这个缝就不存在。
     *
     * 走开机后的单调钟（同 [lastProfileSyncAt] 的理由）：这个值最终要变成上报里的 `premiumAgeMs`，而墙钟被改或被 NTP
     * 拨过之后算出来的时长会是负数甚至几天。 `null` = 本进程还没读到过 —— 冷启到第一次前台同步之间就是这样，那期间上报不带年龄，
     * 服务端按「未知」处理（不会因此掉出池子，只是被拒过的号得等下一次真读到才回得来）。
     *
     * 只记登录账号自己的：借来的号也会被上报，但那份会员状态是借号方经**自己配的 PxveAPI 代理**刷出来的，不该拿它去替别人的号作证。
     */
    @Volatile private var premiumObservation: PremiumObservation? = null
    ...
    /**
     * 冷却时间戳走开机后的单调时钟，不用墙钟：墙钟被用户改或被 NTP 往回拨时， `now - last` 会变成负数，两条冷却判断恒成立，静默同步会一直被冻到时钟追回来 （同类事故见
     * PixivActionQueue 的冷却钳制）。 `null` = 本进程还没同步过，用它和 elapsedRealtime 开机瞬间的 0 区分开—— 否则开机 10 分钟内启动
     * App 会被误判成「刚同步过」。
     */
    @Volatile private var lastProfileSyncAt: Long? = null
    @Volatile private var lastProfileSyncFailedAt: Long? = null
    private val profileSyncInFlight = AtomicBoolean(false)

    /**
     * 把服务端刚拉到的“自己”的资料合并进当前会话并持久化。
     *
     * [uid] 是请求发起时的登录 uid，写入前再和当前会话比对，防止“请求在途时切换了账号” 把上一个账号的资料盖到当前账号头上。必须在主线程调用（内部 setValue 更新
     * LiveData）。
     *
     * 合并结果和旧值一致时直接返回：这条路径每次回前台都会走一遍，无变化还落盘 + 发 LiveData，会让所有观察者（侧边栏、「我的」）白重绑一次。
     */
    @JvmOverloads
     ...
     * **真正归属**。[premium] 是同一响应的 `profile.is_premium`，和 user/detail 的权威字段同源， 由冷启动预热
     * [ceui.pixiv.session.SelfProfileWarmup] 刚从 pixiv 真读到，age≈0。
     *
     * **绝不上报错值的三道闸**（这是本方法存在的全部理由，用户最怕的就是把 A 的会员状态安到 B 上）：
     * 1. `uid <= 0` → 畸形响应（缺 user_id），直接不报；
     * 2. `uid != loggedInUid` → self profile 请求在途时切了号，这份 premium 不属于当前会话，不报。 用响应自带的 user_id
     *    而不是「当前登录 uid」正是为了这一步——后者会把 A 的 premium 报成 B 的；
     * 3. `account.user.id != uid` → 再兜一层，会话账号必须就是这个 uid 才动。 三闸全过之后，上报体直接用 [premium] 覆盖
     *    `is_premium`，不依赖 [ingestFreshUser] 是否真把权威值 写进了会话（它在 `fresh` 不 exist 时会提前
     *    return），保证送出去的**永远**是这份权威值。
     *
     * 和 [ingestFreshUser] 的区别只有一处：那条**只在 is_premium 相对旧值变化时**才上报，服务于 日常静默同步；这里**无条件**报一次 ——
     * 哪怕值没变，也要把一份 `premiumAgeMs≈0` 的新观测送上 去，好让曾被借号方误判过的号凭「更晚的真读」回到借号池（见
     * [ceui.pixiv.shaftapi.BindOnlineReq.premiumAgeMs]）。
     *
     * [markPremiumObserved] 在校验通过后才调：否则会给一个并不属于当前号的观测打上 age 戳。
     *
     * 幂等：outbox 按 `TYPE_ONLINE:uid` 覆盖去重，即便 [ingestFreshUser] 因值变化也报了一次，
     * 两条会并成同一条，不会重复发。必须在主线程调用（内部经 [ingestFreshUser] 更新 LiveData， 且读 [loggedInUid] 与切号的 setValue
     * 同在主线程串行，不会读到半更新态）。
     */
    ...
    /** 前台静默同步：去抖 + 单飞 + 失败静默。头像/昵称在站外被修改后， 回到前台会自动拉一次自己的 user/detail 并写回会话。 */
     ...
     *
     * 只合并可空字段：它们缺省就是 null，能把「服务端没返回」和「服务端返回了空」区分开。 非空且带默认值的字段（如 `gender` 默认 MALE）做不到这个区分，合进来等于拿默认值
     * 覆盖真实值，所以一律不动。
     */
    private fun User.mergedWith(fresh: User, premium: Boolean?): User =
        copy(
    ...
    /**
     * Returns "Bearer xxx" format token for API Authorization header. AccountResponse stores the
     * raw token; HTTP callers use this helper for the Bearer form.
     */
    ...
    /**
     * 见 [SingleFlightTokenRefresher]。锁内会 `withContext(Main)` 回主线程写 LiveData； 主线程永远不会调
     * [refreshAccessToken]，不存在反向拿锁，不会死锁。
     *
     * 已登出（例如并发线程刚因吊销把会话清掉）时 [getAccessToken] 会 throw， 这里换成 null，别把异常抛进 OkHttp 线程。
     */
    private val tokenRefresher =
     ...
     * [ceui.lisa.http.TokenInterceptor]）在 OkHttp 线程上调用：拿着「本次请求用的 access token」来换一个新的。两栈共用这一把锁，同一个旧
     * token 只会触发一次真正的刷新请求。
     *
    ...
    private fun doRefreshLocked(): String? {
        return runBlocking(Dispatchers.IO) {
            try {
                val refreshToken =
                    _loggedInAccount.value?.refresh_token
                ...
                Timber.tag("TokenRefresh")
                    .e(ex, "refresh rejected as INVALID refresh token → signing out + restart")
     ...
     *
     * 判据复用借号那支的 [freshMembershipOf]（uid 对不上就不算数），两处是同一条规则： 一次 token 刷新只能替它自己指名的那个账号说话。
     *
     * 这里多一道 `uid > 0`：[freshMembershipOf] 的调用契约是「uid 已保证 > 0」，而 [loggedInUid] 在未登录/会话还没加载完时就是 0，而
     * pixiv-login 在 pixiv 漏发 id 时 也填 0 —— 两个 0 会相等，等于拿一份认不出主人的响应去改会员状态。
     */
 ...
 *
 * 顺序就是「谁的信息更新」的顺序，而不是「谁更方便拿到」。[stored] 排最后是因为它来自 登录时冻下来的那份 SharedPreferences 副本 —— 全 app
 * 唯一一处从不更新的会员状态， 而借号池正是按上报值决定派不派发的。
 */
